import { getToken, getRefreshToken, setToken, clearToken } from '@/utils/TokenUtil';
...
function parseJSON(response: BaseResponse) {
  if (response.headers['x-total-count']) {
    ...
    response.total = response.headers['x-total-count'] ?? 0;
  }
  ...
  if (response.headers['Access-token']) {
    setToken(response.headers['Access-token'], response.headers['Refresh-token']);
  }
  return response;
}
const getBackLogin = (response: Basic.BaseResponse) => {
  outloginFlag = true;
  Modal.warning({
    ...
    onOk: () => {
      ...
      location.href = `/user/login?revertUrl=${revertUrl}`;
...
function checkStatus(response: BaseResponse) {
  ...
  if (response.status === 401 && response.request.responseURL.indexOf('/auth/logout') === -1) {
    if (!outloginFlag) {
